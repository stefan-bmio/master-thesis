...
Die Reviewregistrierung wird nach regulärer Registrierung ausschließlich manuell über `register.app_review_account = TRUE` markiert. Ereignismails über eine erfolgreiche Aktivierung oder den direkten Abschluss dieses Kontos MÜSSEN im Betreff mit `[TESTKONTO]` und im Nachrichtentext mit `Testkonto: ja` gekennzeichnet sein. Sie DÜRFEN weder die Reviewkennung noch App-Token oder Forschungsdaten enthalten. Registrierung, Feedback und allgemeine Fehlermails werden nicht nachträglich mit dem Reviewkonto verknüpft.
